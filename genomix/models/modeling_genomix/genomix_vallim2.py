...
class GenoMixPreTrainedModel(PreTrainedModel):
...
class GenoMixMamba2Model(GenoMixPreTrainedModel):
    """Vallina Mamba2 + Attention(optional) + MLP(optional)
        ...
        self.layers = nn.ModuleList(
            [
                GenoMixMamba2Block(
                    ...
                    attn_enable=True if i in config.attn_layer_idx else False,
                    attn_cfg=config.attn_cfg,
                    moe_enable=True if i in config.moe_layer_idx else False,
    ...
    def forward(self, input_ids, inference_params=None, **mixer_kwargs):
        ...
        return GenoMixMamba2Output(last_hidden_state=hidden_states)


@dataclass
class GenoMixMamba2ForCausalLMOutput(ModelOutput):
    """
...
class GenoMixMamba2ForCausalLM(GenoMixMamba2PreTrainedModel, GenerationMixin):
    ...
    def forward(self, input_ids, position_ids=None, inference_params=None, **mixer_kwargs):
        ...
        return GenoMixMamba2ForCausalLMOutput(
            logits=logits,
            hidden_states=model_output.hidden_states,
        )
