...
class FloatValueEmbedding(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.value_embedding = nn.Linear(1, d_model, bias=False, **kwargs)

    def forward(self, x):
        return self.value_embedding(x.unsqueeze(-1))
    

class GenoMixTabularEmbedding(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.gene_id_embedding = nn.Embedding(vocab_size, d_model, **kwargs)
        self.gene_val_embedding = FloatValueEmbedding(d_model)

        if fusion_type == 'concat':
            self.fusion = nn.Linear(2*d_model, d_model, bias=False, **kwargs)
        else:
            self.fusion = nn.Identity(**kwargs)
